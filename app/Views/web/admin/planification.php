    ...
    <div class="w-full rounded-2xl mb-6 bg-white border border-gray-200 shadow-sm p-4">
        <div class="flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="hidden sm:flex items-center gap-2 text-sm text-gray-500 bg-gray-50 px-4 py-1.5 rounded-full border border-gray-200 shadow-sm whitespace-nowrap">
                    <span class="material-symbols-outlined text-[16px]">touch_app</span>
                    <span>Cliquer sur une date pour planifier</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <section class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-2xl border border-gray-200">
                ...
                <form id="planning-form" class="px-6 py-6 space-y-5">
                    ...
                    <div class="mt-8 pt-5 border-t border-gray-100 flex items-center justify-end gap-3">
                        <button id="planning-delete" class="hidden px-5 py-2.5 text-sm font-medium text-red-600 bg-white border border-red-200 rounded-xl hover:bg-red-50 focus:outline-none transition-all shadow-sm" type="button">
                            Supprimer
                        </button>
                        <button id="planning-cancel" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 focus:outline-none transition-all shadow-sm" type="button">
                            Annuler
                        </button>
                        <button id="planning-submit" class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-xl hover:bg-blue-700 focus:outline-none transition-all shadow-sm" type="submit">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        busSelect.addEventListener('change', (e) => {
            ...
            if (places && !editingProgrammeId) placesInput.value = places;
        });
        ...
        const refreshSession = async () => {
            const response = await fetch('<?= base_url('session/refresh') ?>', { method: 'POST', headers: { Accept: 'application/json' } });
            const json = await response.json();
            if (!response.ok || json.success === false || !json.data?.access_token) throw new Error(json.message || 'Session expirée.');
        ...
        const clearFormErrors = () => { formErrors.innerHTML = ''; formErrors.classList.add('hidden'); };
        ...
        const errorMessages = (payload) => {
            ...
            if (Array.isArray(errors.conflicts)) {
                return errors.conflicts.map(conflict => {
                    const conducteur = [conflict.conducteur_nom, conflict.conducteur_postnom, conflict.conducteur_prenom].filter(Boolean).join(' ').trim();
                    return `Conflit le ${conflict.date_programme || '-'} : bus ${conflict.numero_plaque || '-'}, conducteur ${conducteur || '-'}.`;
                });
            }
        ...
        const openModal = (date) => {
            ...
            form.querySelectorAll('input[name="jours_semaine[]"]').forEach(cb => cb.checked = true);
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            dateClick: (info) => {
                if (isPastDate(info.dateStr)) { showPageAlert('Impossible de planifier avant la date actuelle.', 'error'); return; }
        ...
        deleteButton.addEventListener('click', async () => {
            ...
            try {
                const response = await apiFetch(`<?= base_url('api/planification') ?>/${editingProgrammeId}`, { method: 'DELETE', headers: { Accept: 'application/json' } });
                const json = await response.json();
                if (!response.ok || json.success === false) { showFormErrors(errorMessages(json)); return; }
                closeModal();
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                const url      = editingProgrammeId ? `<?= base_url('api/planification') ?>/${editingProgrammeId}` : '<?= base_url('api/planification') ?>';
                const method   = editingProgrammeId ? 'PUT' : 'POST';
                const response = await apiFetch(url, { method, headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
                const json     = await response.json();
                if (!response.ok || json.success === false) { showFormErrors(errorMessages(json)); return; }
                closeModal();
                calendar.refetchEvents();
